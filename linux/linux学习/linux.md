...
- 认识linux
- 基本的命令（重点： Git讲了一些基本的命令：文件操作、目录管理、文件属性、Vim编辑器）
- 软件的安装和部署

云服务器就是一台远程的电脑

关于安全组的说明：在阿里云这个很重要，自己需要开放什么端口来这里配置就好

一下就是linux操作系统

![image-20210807234920137](C:\Users\86157\AppData\Roaming\Typora\typora-user-images\image-20210807234920137.png)
...
- cat 由第一行开始显示文件内容，用来读文章或者读命令
- tac 从最后一行开始显示，可以看出 tac 是 cat 的倒着写！
- nl  显示的时候，顺道输出行号！
- more 一页一页的显示文件内容，空格代表翻页，回车代表显示下一行
- less 与 more 类似，但是比 more 更好的是，他可以往前翻页！（空格翻页，上下键代表翻动页面，退出按q）
- head 只看头几行
- tail 只看尾巴几行
...
- 在linux中，每一个程序都是有自己的一个进程，每一个进程都有一个id
- 每一个进程都会有一个父进程
- 进程可以有两种存在方式：前台和后台运行
- 一般的话服务都是后台运行的，基本的程序都是前台运行
...
对于我们来说，这里目前只需要记住一个命令即可 ps -xx|grep 进程名字   来过滤进程信息
