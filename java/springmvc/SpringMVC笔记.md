...
```java
@RequestMapping("/testParam")
public String testParam(@RequestParam(value = "user_name",required = false,defaultValue = "admin") String username, String password) {
...
上面几种共享的数据中都会创建modelAndView，所以这是官方最推荐的方式
...
上面两个标签必须一起使用，如果只有第一个标签，则代表的意思是整个项目只能处理静态资源请求，只有第二个标签的意思是无法获取静态资源
